...
public class Remoting {
    ...
    public ResponseMessage syncCall(RequestMessage requestMessage, Connection connection, int timeoutMills) throws InterruptedException {
        ...
        try {
            ...
            connection.getChannel().writeAndFlush(requestMessage).addListener((ChannelFuture channelFuture) -> {
                if (!channelFuture.isSuccess()) {
                    log.error("Write requestMessage fail. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress(),
                            channelFuture.cause());
            ...
            log.error("Invoke write requestMessage fail. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress(),
                    t);
            InvokeFuture<?> future = connection.removeInvokeFuture(requestId);
            if (future != null) {
        ...
        try {
            ...
            log.warn("Wait responseMessage timeout. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress());
    ...
    public InvokeFuture<? extends ResponseMessage> asyncCall(RequestMessage requestMessage, Connection connection, int timeoutMills) {
        ...
        try {
            ...
            log.error("Invoke write requestMessage fail. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress(), t);
            InvokeFuture<?> future = connection.removeInvokeFuture(requestId);
            if (future != null) {
                future.cancelTimeout();
                future.complete(messageFactory.createSendFailResponseMessage(requestId, t, connection.remoteAddress()));
        ...
        try {
            ...
            connection.getChannel().writeAndFlush(requestMessage).addListener((ChannelFuture channelFuture) -> {
                if (!channelFuture.isSuccess()) {
                    ...
                    if (future != null) {
                        future.cancelTimeout();
                        ResponseMessage responseMessage = messageFactory.createSendFailResponseMessage(requestId, channelFuture.cause(),
                                connection.remoteAddress());
                        future.complete(responseMessage);
                        future.asyncExecuteCallBack(connection.getProtocol().messageHandler().executor(responseMessage));
                    }
            ...
            if (future != null) {
                future.cancelTimeout();
                ResponseMessage responseMessage = messageFactory.createSendFailResponseMessage(requestId, t, connection.remoteAddress())
                future.complete(responseMessage);
                future.asyncExecuteCallBack(connection.getProtocol().messageHandler().executor(responseMessage));
            }
        }
        ...
        try {
            connection.getChannel().writeAndFlush(requestMessage).addListener((ChannelFuture future) -> {
                if (!future.isSuccess()) {
                    ...
                    log.debug("Write requestMessage success. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress());
                }
            });
        } catch (Throwable t) {
            log.error("Invoke write requestMessage fail. requestId:{} remoteAddress:{}", requestId, connection.remoteAddress(), t);
