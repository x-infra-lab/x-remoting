...
public class RemotingMessageHandler extends AbstractMessageHandler {
	...
	// MessageProcessor<RemotingMessage> prevMessageProcessor =
	// messageProcessors.putIfAbsent(rpcMessageType,
	// (MessageProcessor<RemotingMessage>) messageProcessor);
	// if (prevMessageProcessor != null) {
	// throw new RuntimeException("repeat register message processor for " +
	// rpcMessageType);
	// }
	// }
	//
	// public MessageProcessor<RemotingMessage> messageProcessor(RpcMessageType
	// rpcMessageType) {
	// return messageProcessors.get(rpcMessageType);
	// }
	//
	// public void registerUserProcessor(UserProcessor<?> userProcessor) {
	// UserProcessor<?> prevUserProcessor =
	// userProcessors.putIfAbsent(userProcessor.interest(), userProcessor);
	// if (prevUserProcessor != null) {
	// throw new RuntimeException("repeat register user processor for " +
	...
	// }
	//
	// private void exceptionForMessage(MessageHandlerContext messageHandlerContext,
	// RemotingMessage remotingMessage, Throwable t) {
	// RpcMessageType rpcMessageType = remotingMessage.messageType();
	// String errorMsg = String.format("handle %s message fail, id: %s", rpcMessageType,
	// remotingMessage.id());
	// log.error(errorMsg);
